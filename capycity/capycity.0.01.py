...
class SolverTwoDimensions():
    ...
    def incrementResolution(self):                                # !
        ...
        for k in self.M.keys():
            ...
            self.M[k].buildAnchorMask(self.nn, self.ll)
        # debug
...
if __name__ == "__main__":
    ...
    if SELECT == "MULTI-GRID":
        ...
        S = SolverTwoDimensions()
        S.addPart("S", DiskAperture(0.45))
        S.addPart("C", PlateSolid(0.0, 0.0, 0.5, 0.5))
        
        i, c = S.jacobiSteps(
            20,
            "S", "C",
            SavePattern = list(range(21)))

        # MAKE FIGURE
        fg, ax = selectfigure("PLOT")
        ax.plot(i, array(c)*1E12, ".-k")

        # EXPORT PLOT RESULT AS PDF
        D = Document()
        D.opendocument("../local/plot.pdf")
        D.exportfigure("PLOT")
        D.closedocument()
